<x-doctor-layout>
    ...
    <div x-data="prescriptionSetup({{ $medications->toJson() }})" class="max-w-7xl mx-auto pb-12">
        ...
        <div class="flex justify-center w-full">
            <!-- The A4 Canvas -->
            <div id="prescription-print-area" class="bg-white shadow-2xl border border-gray-200 max-w-3xl w-full mx-auto flex flex-col relative overflow-hidden aspect-[1/1.414] print:aspect-auto print:w-full print:h-full print:block print:absolute print:inset-0 print:m-0 print:p-0 print:border-none print:bg-white text-gray-900">

                <!-- Header Section -->
                <div class="flex justify-between items-center px-10 pt-10 pb-4 bg-white print:bg-transparent">
                    <!-- Stethoscope Icon -->
                    <div class="w-24 h-24 text-blue-600 print:text-blue-600">
                        @if($settings->logo_1_path)
                            <img src="{{ Storage::url($settings->logo_1_path) }}" alt="Logo" class="w-full h-full object-contain">
                        @else
                    ...
                    <div class="text-right">
                        ...
                        <h2 class="text-sm font-semibold tracking-widest text-slate-600 print:text-black mb-2 uppercase">{{ __('QUALIFICATION') }}</h2>

                        <div class="h-0.5 w-full bg-blue-200 print:bg-blue-200 mb-2"></div>

                        <div class="flex justify-between items-end">
                            <p class="text-[10px] text-gray-500 print:text-gray-600">{{ __('Certification 12548-20') }}</p>
                            <div class="text-right">
                                <h3 class="text-xl font-bold text-blue-600 print:text-blue-600 leading-tight">{{ $settings->clinic_name }}</h3>
                                <p class="text-[10px] text-gray-500 print:text-gray-600 uppercase">{{ __('SLOGAN HERE') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-b from-blue-50 to-transparent print:from-blue-50 print:to-transparent px-10 py-6 text-sm font-medium">
                    <div class="grid grid-cols-12 gap-y-4 gap-x-6 items-end">
                        ...
                        <div class="col-span-12 flex items-center gap-2">
                            <span class="text-gray-700 w-24 shrink-0">{{ __('Address:') }}</span>
                        ...
                        <div class="col-span-6 flex items-center gap-2">
                            <span class="text-gray-700 w-12 shrink-0">{{ __('Date:') }}</span>
                            <div class="flex-1 border-b border-blue-200 relative h-6">
                                <span class="absolute bottom-1 px-2" x-text="bookingDate || '{{ today()->format('Y/m/d') }}'" dir="ltr"></span>
                            </div>
                        </div>
                ...
                <!-- Body (Rx & Medications) -->
                <div class="flex-1 px-10 py-6 flex flex-col relative z-10">
                    <!-- Rx Logo -->
                    <div class="mb-6">
                        <svg class="w-12 h-12 text-blue-500 print:text-blue-500" viewBox="0 0 24 24" fill="currentColor">
                    ...
                    <div class="space-y-6 min-h-[220px]">
                        <!-- Empty State -->
                        <div x-show="addedMedications.length === 0" class="text-center text-slate-400 print:hidden mt-12 text-sm">
                            {{ __('قم بإضافة أدوية من القائمة الجانبية') }}
                        </div>

                        <!-- Medication List -->
                        <template x-for="(med, index) in addedMedications" :key="index">
                            <div class="relative group border-b border-blue-50 print:border-transparent pb-4 last:border-0 hover:bg-slate-50 print:hover:bg-transparent -mx-3 px-3 rounded-lg transition-colors">
                                ...
                                <div class="flex items-start gap-4">
                                    <div class="text-lg font-bold text-gray-800 print:text-black mt-0.5 w-6 text-right" x-text="(index + 1) + '.'"></div>
                                    <div class="flex-1">
                                        <div class="text-lg font-bold text-gray-900 print:text-black flex items-center gap-2">
                                            <span x-text="med.name"></span>
                                            <span x-show="med.type" class="text-xs px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full print:border print:border-black print:bg-transparent" x-text="med.type"></span>
                                        </div>
                                        <div x-show="med.generic" class="text-sm text-gray-500 print:text-gray-700 mb-2 italic" x-text="med.generic"></div>

                                        <!-- Editable Dosage and Usage -->
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                                            <div>
                                                <label class="block text-xs font-semibold text-gray-400 print:hidden mb-1">{{ __('الجرعة') }}</label>
                                                <input type="text" x-model="med.dosage" placeholder="{{ __('مثال') }}: {{ __('حبة واحدة') }}" class="w-full bg-transparent border-b border-gray-200 print:border-transparent focus:border-blue-500 focus:outline-none focus:ring-0 text-gray-800 print:text-black text-sm px-0 py-1 transition-colors font-medium">
                                            </div>
                                            <div>
                                                <label class="block text-xs font-semibold text-gray-400 print:hidden mb-1">{{ __('وقت الاستخدام') }}</label>
                                                <input type="text" x-model="med.usage" placeholder="{{ __('مثال') }}: {{ __('مرتين يومياً بعد الأكل') }}" class="w-full bg-transparent border-b border-gray-200 print:border-transparent focus:border-blue-500 focus:outline-none focus:ring-0 text-gray-800 print:text-black text-sm px-0 py-1 transition-colors font-medium">
                                            </div>
                                            <div class="md:col-span-2">
                                                <label class="block text-xs font-semibold text-gray-400 print:hidden mb-1">{{ __('دواعي الاستعمال') }}</label>
                                                <input type="text" x-model="med.indications" placeholder="{{ __('مثال') }}: {{ __('مسكن للألم') }}" class="w-full bg-transparent border-b border-gray-200 print:border-transparent focus:border-blue-500 focus:outline-none focus:ring-0 text-gray-800 print:text-black text-sm px-0 py-1 transition-colors font-medium">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <!-- Doctor's Notes -->
                    <div class="mt-8 pt-4 border-t border-dashed border-gray-200 print:border-black">
                        <label class="block text-sm font-bold text-gray-700 print:hidden mb-2">{{ __('ملاحظات الطبيب') }}:</label>
                        <textarea rows="3" class="w-full bg-gray-50 print:bg-transparent border border-gray-200 print:border-0 rounded-lg p-4 text-gray-800 print:text-black focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-none font-medium text-sm leading-relaxed" placeholder="{{ __('اكتب ملاحظاتك هنا') }}..."></textarea>
                    </div>
                </div>

                <!-- Footer -->
                <div class="mt-auto px-10 pb-8 pt-4 relative overflow-hidden text-gray-600 print:text-black text-xs font-medium border-t-4 border-blue-500 print:border-black">
                    ...
                    <div class="grid grid-cols-2 gap-4 relative z-10">
                        <div class="space-y-2">
                            <div class="flex items-center gap-2">
                                ...
                                <span>55 47 79 94 15<br>55 47 79 94 18</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <svg class="w-4 h-4 text-blue-500 print:text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                                <span>{{ __('Address Here Number 123') }}</span>
                            </div>
                        </div>
                        <div class="space-y-2 flex flex-col justify-end">
                            <div class="flex items-center gap-2">
                                <svg class="w-4 h-4 text-blue-500 print:text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                                <span>email_here@email.com</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <svg class="w-4 h-4 text-blue-500 print:text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"></path></svg>
                                <span>www.webpage.com</span>
                            </div>
                        </div>
                    </div>
                </div>
