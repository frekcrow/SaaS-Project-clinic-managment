            ...
            <div class="lg:col-span-1 print:hidden space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    ...
                    <form action="{{ route('doctor.prescriptions.settings.update') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">اسم العيادة</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">اسم الطبيب</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">الشعار الأول (اليمين)</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">الشعار الثاني (اليسار - اختياري)</label>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    ...
                    <div class="space-y-4">
                        <!-- Patient Selection -->
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">المريض (مواعيد اليوم)</label>
                            <select x-model="selectedAppointmentId" @change="updatePatientData" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-colors">
                                ...
                                @foreach($appointments as $app)
                                    <option value="{{ $app->id }}" data-patient="{{ $app->patient_name ?? ($app->patient ? $app->patient->name : '') }}" data-date="{{ $app->appointment_date ? $app->appointment_date->format('Y/m/d') : today()->format('Y/m/d') }}" data-booking="{{ $app->queue_number ?? $app->id }}">
                                        {{ $app->patient_name ?? ($app->patient ? $app->patient->name : 'بدون اسم') }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div x-show="selectedAppointmentId" x-collapse>
                            <div class="p-3 bg-slate-50 rounded-xl space-y-3 mt-2 border border-slate-100">
                                <div>
                                    <label class="block text-xs font-medium text-slate-500 mb-1">اسم المريض</label>
                                    <input type="text" x-model="patientName" class="w-full bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-sm">
                                </div>
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        <label class="block text-xs font-medium text-slate-500 mb-1">رقم الحجز</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">إضافة دواء</label>
            ...
            <div class="lg:col-span-2">
                <div class="sticky top-6">
                    ...
                    <div class="flex justify-between items-center mb-4 print:hidden">
                        <h2 class="text-xl font-bold text-slate-800">معاينة الوصفة</h2>
                    ...
                    <div class="bg-white rounded-lg shadow-xl print:shadow-none w-full max-w-[800px] mx-auto overflow-hidden flex flex-col relative aspect-[1/1.414] print:aspect-auto print:w-full print:h-full print:block">
                        ...
                        <div class="flex justify-between items-start p-8 border-b-2 border-slate-800 print:border-black">
                            ...
                            <div class="flex-1 flex flex-col items-center justify-center text-center px-4">
                                ...
                                <h2 class="text-xl font-bold text-slate-700 print:text-black mt-2">د. {{ $settings->doctor_name }}</h2>
                            </div>
                        ...
                        <div class="bg-slate-50 print:bg-transparent border-b border-slate-200 print:border-black py-3 px-8 flex justify-between items-center text-sm font-semibold">
                            <div class="flex items-center gap-2 text-slate-800 print:text-black">
                                <span class="text-slate-500 print:text-gray-600">المريض:</span>
                                <span x-text="patientName || '...................................'"></span>
                            </div>
                            <div class="flex items-center gap-6">
                                <div class="flex items-center gap-2 text-slate-800 print:text-black">
                                    <span class="text-slate-500 print:text-gray-600">رقم الحجز:</span>
                                    <span x-text="bookingNumber || '.........'" dir="ltr"></span>
                                </div>
                                <div class="flex items-center gap-2 text-slate-800 print:text-black">
                                    <span class="text-slate-500 print:text-gray-600">التاريخ:</span>
                        ...
                        <div class="flex-1 p-8">
                            ...
                            <div class="space-y-6 min-h-[300px]">
                                ...
                                <div x-show="addedMedications.length === 0" class="text-center text-slate-400 print:hidden mt-20">
                                    قم بإضافة أدوية من القائمة الجانبية
                                </div>
                                        ...
                                        <div class="flex items-start gap-4">
                                            ...
                                            <div class="flex-1">
                                                ...
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                                                    <div>
                                                        <label class="block text-xs font-semibold text-slate-400 print:hidden mb-1">الجرعة (Dosage)</label>
                                                        <input type="text" x-model="med.dosage" placeholder="مثال: حبة واحدة" class="w-full bg-transparent border-b border-slate-200 print:border-transparent focus:border-teal-500 focus:outline-none focus:ring-0 text-slate-800 print:text-black text-sm px-0 py-1 transition-colors font-medium">
                                                    </div>
                                                    <div>
                                                        <label class="block text-xs font-semibold text-slate-400 print:hidden mb-1">وقت الاستخدام (Usage)</label>
                                                        <input type="text" x-model="med.usage" placeholder="مثال: مرتين يومياً بعد الأكل" class="w-full bg-transparent border-b border-slate-200 print:border-transparent focus:border-teal-500 focus:outline-none focus:ring-0 text-slate-800 print:text-black text-sm px-0 py-1 transition-colors font-medium">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="mt-8 border-t border-slate-200 print:border-black pt-4">
                                <label class="block text-sm font-bold text-slate-700 print:hidden mb-2">ملاحظات الطبيب:</label>
                                <textarea rows="4" class="w-full bg-slate-50 print:bg-transparent border border-slate-200 print:border-0 rounded-xl p-4 text-slate-800 print:text-black focus:ring-2 focus:ring-teal-500 focus:border-teal-500 resize-none font-medium text-sm leading-relaxed" placeholder="اكتب ملاحظاتك هنا..."></textarea>
                            </div>
                        </div>
