    ...
    <div class="py-12" x-data="doctorSurgeriesGrid()">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <template x-for="surgery in filteredSurgeries" :key="surgery.id">
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] overflow-hidden transition-all duration-300" x-data="{ expanded: false }">
                        ...
                        <div class="p-6 cursor-pointer hover:bg-slate-50 transition-colors" @click="expanded = !expanded">
                            <div class="flex justify-between items-center">
                                <div>
                                    <h3 class="text-lg font-bold text-slate-800 mb-1" x-text="surgery.patient ? surgery.patient.name : 'مريض غير معروف'"></h3>
                        ...
                        <div x-show="expanded" x-collapse x-cloak class="border-t border-gray-100 bg-slate-50/50 p-6">
                            <form :action="`/doctor/surgeries/${surgery.id}`" method="POST">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <!-- Hospital Name -->
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 mb-1">المستشفى</label>
                                    ...
                                    <!-- Surgeon Name -->
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 mb-1">الجراح</label>
                                    ...
                                    <!-- Disease Name -->
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 mb-1">التشخيص / المرض</label>
                                    ...
                                    <!-- Assistant Name -->
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 mb-1">المساعد</label>
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 mb-1">طبيب التخدير</label>
                                    ...
                                    <!-- Anesthesia Type -->
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 mb-1">نوع التخدير</label>
                                        <select name="anesthesia_type" required class="w-full border-gray-300 rounded-xl focus:ring-teal-500 focus:border-teal-500 sm:text-sm">
                                            <option value="تخدير عام" :selected="surgery.anesthesia_type === 'تخدير عام'">تخدير عام</option>
                                            <option value="تخدير موضعي" :selected="surgery.anesthesia_type === 'تخدير موضعي'">تخدير موضعي</option>
                                            <option value="تخدير قطني" :selected="surgery.anesthesia_type === 'تخدير قطني'">تخدير قطني</option>
                                            <option value="أخرى" :selected="surgery.anesthesia_type === 'أخرى'">أخرى</option>
                                        </select>
                                    </div>

                                    <!-- Cost -->
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 mb-1">التكلفة</label>
                                ...
                                <!-- Shared Notes -->
                                <div class="mt-4">
                                    <label class="block text-sm font-medium text-slate-700 mb-1">ملاحظات مشتركة</label>
                                ...
                                <div class="mt-4 p-4 bg-teal-50/50 rounded-xl border border-teal-100">
                                    <label class="block text-sm font-bold text-teal-800 mb-1 flex items-center gap-2">
                                        ...
                                        ملاحظات الطبيب الخاصة (مخفية عن السكرتارية)
                                    </label>
                                    <textarea name="doctor_notes" rows="3" class="w-full border-teal-200 bg-white rounded-xl focus:ring-teal-500 focus:border-teal-500 sm:text-sm" placeholder="أدخل أي ملاحظات خاصة بك هنا..." x-text="surgery.doctor_notes"></textarea>
                                </div>
            ...
            <div x-show="filteredSurgeries.length === 0" x-cloak class="bg-white rounded-3xl p-12 border border-gray-100 shadow-sm flex flex-col items-center justify-center text-center">
                ...
                <h3 class="text-xl font-bold text-slate-700 mb-2">لا توجد عمليات</h3>
                <p class="text-gray-500 max-w-sm">لم يتم العثور على أي عمليات مطابقة لبحثك.</p>
            </div>
        </div>
    </div>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('doctorSurgeriesGrid', () => ({
                ...
                formatDate(dateString) {
                    ...
                    return `موعد العملية : ${year}/${month}/${day}`;
                }
            }));
        });
    </script>
