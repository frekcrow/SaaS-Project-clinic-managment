            ...
            >
                ...
                <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto overflow-x-hidden custom-scrollbar">
                    ...
                    <div x-data="{ openSupport: false }" class="relative">
                        <button @click="openSupport = !openSupport" class="w-full flex items-center gap-3 px-3 py-2 text-slate-300 hover:bg-slate-800 hover:text-white rounded-2xl transition-all duration-200 group">
                    ...
                    <a href="{{ route('api.notifications.index') }}" class="flex items-center gap-3 px-3 py-2 text-slate-300 hover:bg-slate-800 hover:text-white rounded-2xl transition-all duration-200 group">
                    ...
                    <!-- Messages -->
                    <a href="{{ route('chat.index') }}" class="flex items-center gap-3 px-3 py-2 text-slate-300 hover:bg-slate-800 hover:text-white rounded-2xl transition-all duration-200 group {{ request()->routeIs('chat.*') ? 'bg-slate-800 text-teal-400' : '' }}">
                        <div class="flex items-center justify-center w-10 h-10 rounded-xl bg-slate-800/50 shadow-sm border border-slate-700 group-hover:border-slate-600 group-hover:shadow transition-all flex-shrink-0 {{ request()->routeIs('chat.*') ? 'border-slate-600 shadow bg-slate-800' : '' }}">
                            <svg class="w-5 h-5 transition-colors {{ request()->routeIs('chat.*') ? 'text-teal-400' : 'text-slate-400 group-hover:text-teal-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
                        </div>
                        <span x-show="!isCollapsed" x-transition.opacity.duration.300ms class="font-medium whitespace-nowrap">{{ __('الرسائل') }}</span>
                    </a>
                </nav>
            </aside>
