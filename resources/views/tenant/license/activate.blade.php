...
<body class="font-sans text-gray-900 antialiased selection:bg-gray-900 selection:text-white">
    <div class="min-h-screen flex flex-col items-center justify-center p-4 bg-slate-50 relative overflow-hidden">
        ...
        <div class="relative z-10 flex flex-col items-center text-center mb-8 max-w-md w-full">
            ...
            <h2 class="text-3xl font-bold mb-2 font-sans text-slate-900 tracking-tight" style="font-family: 'Ping', sans-serif;">
            ...
            @if (session('success'))
                <div class="w-full mb-6 p-4 text-sm text-emerald-800 rounded-2xl bg-emerald-50 border border-emerald-200/60 shadow-sm text-center" role="alert">
                    {{ session('success') }}
                </div>
            @endif
            @if (session('error'))
                <div class="w-full mb-6 p-4 text-sm text-rose-800 rounded-2xl bg-rose-50 border border-rose-200/60 shadow-sm text-center" role="alert">
                    {{ session('error') }}
                </div>
            @endif
        </div>
        ...
        <form method="POST" action="{{ route('activation.submit') }}" class="w-full max-w-md relative z-10 flex flex-col items-center">
            ...
            <div class="w-full max-w-md aspect-[1.586/1] rounded-3xl p-8 relative overflow-hidden shadow-2xl bg-gradient-to-br from-gray-900 via-slate-800 to-black text-white flex flex-col justify-between border border-white/10">
                ...
                <div class="w-12 h-12 rounded-full absolute top-1/2 -right-6 transform -translate-y-1/2 bg-slate-50 pointer-events-none z-20"></div>

                <!-- Top Row: Smart Chip & Atlas Branding -->
                <div class="flex items-center justify-between relative z-10">
                    <!-- Smart Chip Icon -->
                    <div class="w-11 h-8 rounded-md bg-gradient-to-tr from-amber-300 via-amber-400 to-yellow-200 p-0.5 shadow-sm opacity-90 flex items-center justify-center relative border border-amber-500/30">
                ...
                <div class="relative z-10 my-auto pt-4">
                    ...
                    <input id="activation_code"
                           ...
                           class="w-full bg-transparent border-b border-white/20 focus:border-white/70 text-white text-2xl tracking-widest placeholder:text-gray-600 focus:outline-none focus:ring-0 py-2 transition transition-colors font-mono" />
            ...
            <button type="submit"
                    class="mt-8 w-full max-w-md bg-black text-white font-medium py-3.5 px-6 rounded-2xl hover:bg-slate-800 active:scale-[0.99] transition duration-200 text-center shadow-lg shadow-black/5 flex items-center justify-center text-base cursor-pointer">
                متابعة
            </button>
        </form>
    </div>
