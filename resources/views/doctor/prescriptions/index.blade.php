                    ...
                    <div id="prescription-print-area" class="bg-white rounded-lg shadow-xl print:shadow-none w-full max-w-xl mx-auto overflow-hidden flex flex-col relative aspect-[1/1.414] print:aspect-auto print:w-full print:h-full print:block print:absolute print:inset-0 print:m-0 print:p-0 print:border-none print:bg-white">
                        ...
                        <div class="flex justify-between items-start p-5 border-b-2 border-slate-800 print:border-black">
                            ...
                            <div class="w-24 flex flex-col items-center">
                                @if($settings->logo_1_path)
                                    <img src="{{ Storage::url($settings->logo_1_path) }}" alt="Logo" class="h-16 object-contain">
                                @endif
                            </div>

                            <!-- Center Info -->
                            <div class="flex-1 flex flex-col items-center justify-center text-center px-3">
                                <h1 class="text-2xl font-extrabold text-slate-900 print:text-black tracking-tight" style="font-family: 'Tajawal', sans-serif;">{{ $settings->clinic_name }}</h1>
                                <h2 class="text-base font-bold text-slate-700 print:text-black mt-1">{{ __('د') }}. {{ $settings->doctor_name }}</h2>
                            </div>

                            <!-- Logo 2 (Left) -->
                            <div class="w-24 flex flex-col items-center">
                                @if($settings->logo_2_path)
                                    <img src="{{ Storage::url($settings->logo_2_path) }}" alt="Logo" class="h-16 object-contain">
                                @endif
                            </div>
                        </div>

                        <!-- Patient Info Bar -->
                        <div class="bg-slate-50 print:bg-transparent border-b border-slate-200 print:border-black py-2.5 px-6 flex justify-between items-center text-xs font-semibold">
                            ...
                            <div class="flex items-center gap-4">
                                <div class="flex items-center gap-1.5 text-slate-800 print:text-black">
                        ...
                        <div class="flex-1 p-6">
                            <div class="mb-3">
                                <svg class="w-7 h-7 text-slate-800 print:text-black" viewBox="0 0 24 24" fill="currentColor">
                            ...
                            <div class="space-y-4 min-h-[220px]">
                                ...
                                <template x-for="(med, index) in addedMedications" :key="index">
                                    <div class="relative group border-b border-slate-100 print:border-transparent pb-3 last:border-0 hover:bg-slate-50 print:hover:bg-transparent -mx-3 px-3 rounded-lg transition-colors">
                                        ...
                                        <div class="flex items-start gap-3">
                                            <div class="text-base font-bold text-slate-800 print:text-black mt-0.5" x-text="(index + 1) + ' -'"></div>
                                            <div class="flex-1">
                                                <div class="text-base font-bold text-slate-900 print:text-black flex items-center gap-2">
                                                    <span x-text="med.name"></span>
                                                    <span x-show="med.type" class="text-[10px] px-2 py-0.5 bg-slate-100 text-slate-600 rounded-full print:border print:border-black print:bg-transparent" x-text="med.type"></span>
                                                </div>
                                                <div x-show="med.generic" class="text-xs text-slate-500 print:text-gray-700 mb-1.5 italic" x-text="med.generic"></div>

                                                <!-- Editable Dosage and Usage -->
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mt-1.5">
                                                    <div>
                                                        <label class="block text-[11px] font-semibold text-slate-400 print:hidden mb-0.5">{{ __('الجرعة') }}</label>
                                                        <input type="text" x-model="med.dosage" placeholder="{{ __('مثال') }}: {{ __('حبة واحدة') }}" class="w-full bg-transparent border-b border-slate-200 print:border-transparent focus:border-teal-500 focus:outline-none focus:ring-0 text-slate-800 print:text-black text-xs px-0 py-0.5 transition-colors font-medium">
                                                    </div>
                                                    <div>
                                                        <label class="block text-[11px] font-semibold text-slate-400 print:hidden mb-0.5">{{ __('وقت الاستخدام') }}</label>
                                                        <input type="text" x-model="med.usage" placeholder="{{ __('مثال') }}: {{ __('مرتين يومياً بعد الأكل') }}" class="w-full bg-transparent border-b border-slate-200 print:border-transparent focus:border-teal-500 focus:outline-none focus:ring-0 text-slate-800 print:text-black text-xs px-0 py-0.5 transition-colors font-medium">
                                                    </div>
                                                    <div class="md:col-span-2">
                                                        <label class="block text-[11px] font-semibold text-slate-400 print:hidden mb-0.5">{{ __('دواعي الاستعمال') }}</label>
                                                        <input type="text" x-model="med.indications" placeholder="{{ __('مثال') }}: {{ __('مسكن للألم') }}" class="w-full bg-transparent border-b border-slate-200 print:border-transparent focus:border-teal-500 focus:outline-none focus:ring-0 text-slate-800 print:text-black text-xs px-0 py-0.5 transition-colors font-medium">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <!-- Doctor's Notes -->
                            <div class="mt-6 border-t border-slate-200 print:border-black pt-3">
                                <label class="block text-xs font-bold text-slate-700 print:hidden mb-1.5">{{ __('ملاحظات الطبيب') }}:</label>
                                <textarea rows="3" class="w-full bg-slate-50 print:bg-transparent border border-slate-200 print:border-0 rounded-lg p-3 text-slate-800 print:text-black focus:ring-2 focus:ring-teal-500 focus:border-teal-500 resize-none font-medium text-xs leading-relaxed" placeholder="{{ __('اكتب ملاحظاتك هنا') }}..."></textarea>
                            </div>
                        </div>

                        <!-- Footer Branding -->
                        <div class="p-4 bg-slate-50 print:bg-transparent mt-auto text-center border-t border-slate-200 print:border-black">
                            <p class="text-[11px] font-bold text-slate-400 print:text-gray-500 tracking-wider">
