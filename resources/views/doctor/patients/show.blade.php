<x-doctor-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center print:hidden">
            ...
            <button onclick="window.print()" class="flex items-center gap-2 bg-black text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-neutral-800 transition-colors shadow-sm print:hidden">
        ...
        <div class="hidden print:flex flex-col items-center justify-center border-b-2 border-black pb-4 mb-8">
            <h1 class="text-3xl font-bold mb-2">عيادة الطبيب</h1>
            <h2 class="text-xl">السجل الطبي الإلكتروني (EMR)</h2>
            <div class="mt-4 flex gap-8 text-sm">
                <span>تاريخ الطباعة: {{ now()->format('Y-m-d') }}</span>
                <span>المريض: {{ $patient->name }}</span>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-white rounded-3xl p-8 border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] print:shadow-none print:border-black/20 print:rounded-none">
                <h3 class="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2 print:border-b print:pb-2 print:border-black/20">
                    ...
                    المعلومات الشخصية والتاريخ المرضي
                </h3>
                ...
                <form action="{{ route('doctor.patients.update', $patient) }}" method="POST" class="print:hidden">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <select name="gender" class="w-full border-gray-300 focus:border-slate-500 focus:ring-slate-500 rounded-xl shadow-sm">
                        ...
                        <div class="col-span-full mt-4 border-t border-gray-100 pt-4">
                            <h4 class="text-sm font-bold text-slate-600 mb-4">التاريخ الطبي (Medical History)</h4>
                        </div>

                        <div class="col-span-1 md:col-span-2 lg:col-span-1">
                            <label class="block text-sm font-medium text-gray-700 mb-1">الحساسية (Allergies)</label>
                        ...
                        <div class="col-span-1 md:col-span-2 lg:col-span-1">
                            <label class="block text-sm font-medium text-gray-700 mb-1">الأدوية المنتظمة</label>
                        ...
                        <div class="col-span-1 md:col-span-3">
                            <label class="block text-sm font-medium text-gray-700 mb-1">سبب الزيارة والأعراض (من السكرتير)</label>
                            <div class="grid grid-cols-2 gap-4">
                                <textarea name="reason_for_visit" rows="2" placeholder="سبب الزيارة..." class="w-full border-gray-300 focus:border-slate-500 focus:ring-slate-500 rounded-xl shadow-sm">{{ $patient->reason_for_visit }}</textarea>
                                <textarea name="symptoms_onset" rows="2" placeholder="بداية الأعراض..." class="w-full border-gray-300 focus:border-slate-500 focus:ring-slate-500 rounded-xl shadow-sm">{{ $patient->symptoms_onset }}</textarea>
                            </div>
                        </div>
                    </div>
                ...
                <div class="hidden print:block space-y-4">
                    <div class="grid grid-cols-3 gap-4 border-b border-gray-200 pb-4">
                        <div><strong class="text-gray-600 block">الاسم:</strong> {{ $patient->name }}</div>
                        <div><strong class="text-gray-600 block">العمر:</strong> {{ $patient->dob ? \Carbon\Carbon::parse($patient->dob)->age . ' سنة' : '-' }}</div>
                        <div><strong class="text-gray-600 block">الجنس:</strong> {{ $patient->gender == 'male' ? 'ذكر' : ($patient->gender == 'female' ? 'أنثى' : '-') }}</div>
                        <div><strong class="text-gray-600 block">فصيلة الدم:</strong> {{ $patient->blood_type ?: '-' }}</div>
                        <div><strong class="text-gray-600 block">رقم الهاتف:</strong> {{ $patient->phone ?: '-' }}</div>
                    </div>
                    <div class="grid grid-cols-2 gap-4 pt-2">
                        <div><strong class="text-gray-600 block mb-1">الحساسية:</strong> <p class="text-sm">{{ $patient->allergies ?: 'لا يوجد' }}</p></div>
                        <div><strong class="text-gray-600 block mb-1">الأمراض المزمنة:</strong> <p class="text-sm">{{ $patient->chronic_diseases ?: 'لا يوجد' }}</p></div>
                        <div class="col-span-2"><strong class="text-gray-600 block mb-1">الأدوية المنتظمة:</strong> <p class="text-sm">{{ $patient->regular_medications ?: 'لا يوجد' }}</p></div>
                        <div class="col-span-2"><strong class="text-gray-600 block mb-1">سبب الزيارة والأعراض:</strong> <p class="text-sm">{{ $patient->reason_for_visit }} - {{ $patient->symptoms_onset }}</p></div>
                    </div>
                </div>
            </div>
            ...
            @php
                $currentRecord = $patient->medicalRecords->where('diagnosis', 'قيد الانتظار')->first() ?? $patient->medicalRecords->first();
            @endphp
            <div class="bg-white rounded-3xl p-8 border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] print:shadow-none print:border-black/20 print:rounded-none">
                <h3 class="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2 print:border-b print:pb-2 print:border-black/20">
                    ...
                    التشخيص والوصفة الطبية (Current Visit)
                </h3>
                ...
                <form action="{{ route('doctor.patients.records.store', $patient) }}" method="POST" class="print:hidden">
                    ...
                    <div class="space-y-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">التشخيص (Diagnosis)</label>
                            <textarea name="diagnosis" rows="3" class="w-full border-gray-300 focus:border-slate-500 focus:ring-slate-500 rounded-xl shadow-sm">{{ $currentRecord && $currentRecord->diagnosis !== 'قيد الانتظار' ? $currentRecord->diagnosis : '' }}</textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">الوصفة الطبية والنصائح (Prescription & Advice)</label>
                ...
                <div class="hidden print:block space-y-6 mt-4">
                    <div>
                        <strong class="text-xl border-b-2 border-black inline-block mb-2 pr-8 pb-1">التشخيص (Diagnosis):</strong>
                        <p class="text-lg mt-2 whitespace-pre-line">{{ $currentRecord && $currentRecord->diagnosis !== 'قيد الانتظار' ? $currentRecord->diagnosis : 'لم يتم التشخيص بعد' }}</p>
                    </div>
                    <div class="pt-4">
                        <strong class="text-xl border-b-2 border-black inline-block mb-2 pr-8 pb-1">الوصفة الطبية (Rx):</strong>
            ...
            <div x-data="{ activeTab: 'xray' }" class="bg-white rounded-3xl p-8 border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] print:hidden">
                ...
                <div class="flex gap-4 mb-6 border-b border-gray-100">
                    <button @click="activeTab = 'xray'" :class="{'border-black text-black': activeTab === 'xray', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'xray'}" class="pb-3 border-b-2 font-medium text-sm transition-colors">صور الأشعة</button>
                    <button @click="activeTab = 'prescription'" :class="{'border-black text-black': activeTab === 'prescription', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'prescription'}" class="pb-3 border-b-2 font-medium text-sm transition-colors">وصفات سابقة</button>
                    <button @click="activeTab = 'diagnostic'" :class="{'border-black text-black': activeTab === 'diagnostic', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'diagnostic'}" class="pb-3 border-b-2 font-medium text-sm transition-colors">صور تشخيصية</button>
                </div>
                                    ...
                                    <div class="group relative rounded-2xl overflow-hidden border border-gray-200 aspect-square bg-gray-50 flex items-center justify-center">
                                        ...
                                        <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center p-4">
                                            <form action="{{ route('doctor.patients.images.destroy', [$patient, $image]) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="bg-red-500 text-white p-2 rounded-full hover:bg-red-600 transition-colors shadow-sm mb-2" onclick="return confirm('هل أنت متأكد من حذف هذه الصورة؟')">
