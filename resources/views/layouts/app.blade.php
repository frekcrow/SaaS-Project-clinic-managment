        ...
        <div x-data="{ isCollapsed: {{ request()->routeIs('dashboard') ? 'false' : 'true' }} }" class="flex h-screen bg-gray-50 overflow-hidden">
            ...
            >
                ...
                <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto overflow-x-hidden">
                    ...
                    <!-- Messages -->
                    <a href="{{ route('chat.index') }}" class="flex items-center gap-3 px-3 py-2 text-slate-600 hover:bg-slate-100/50 hover:text-indigo-600 rounded-2xl transition-all duration-200 group {{ request()->routeIs('chat.*') ? 'bg-indigo-50 text-indigo-600' : '' }}">
                        <div class="flex items-center justify-center w-10 h-10 rounded-xl bg-white shadow-sm border border-slate-100 group-hover:border-indigo-200 group-hover:shadow transition-all flex-shrink-0 {{ request()->routeIs('chat.*') ? 'border-indigo-200 shadow' : '' }}">
                            <svg class="w-5 h-5 transition-colors {{ request()->routeIs('chat.*') ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
                        </div>
